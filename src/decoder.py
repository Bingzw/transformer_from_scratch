...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self,
        ...
        output = self.dropout2(
            self.cross_mha.forward(x, encoder_hidden_states=encoder_hidden_states, src_padding_mask=src_padding_mask)
        )
...
class TransformerDecoder(nn.Module):
    def __init__(self, embedding: torch.nn.Embedding, hidden_dim: int, ff_dim: int, num_heads: int, num_layers: int,
                 vocab_size: int, dropout_p: float, tie_output_to_embedding: Optional[bool] = True):
        super().__init__()
    ...
    def forward(self, input_tokens: torch.IntTensor, encoder_hidden_states: torch.Tensor,
        ...
        """
        x = self.embed(input_tokens) * math.sqrt(self.hidden_dim)  # (N, T, E)
